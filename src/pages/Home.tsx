...
function Home() {
  const [dersler, setDersler] = createSignal<
    Array<{ id: number; name: string; monthlyfee: number }>
  >([]);
  const [loading, setLoading] = createSignal(true);

  const [newDersName, setNewDersName] = createSignal("");
  const [adding, setAdding] = createSignal(false);
  const [newDersFee, setNewDersFee] = createSignal(0);

  const [editingId, setEditingId] = createSignal<number | null>(null);
  const [editName, setEditName] = createSignal("");
  const [editDersFee, setEditDersFee] = createSignal(0);
  ...
  const handleAddDers = async (e: Event) => {
    ...
    setAdding(true);
    try {
      await invoke("add_ders", {
        name: newDersName(),
        monthlyfee: newDersFee(),
      });
      setNewDersName("");
      fetchDersler();
  ...
  return (
    <main class="container mx-auto p-6 relative">
      ...
      >
        ...
        <input
          type="number"
          placeholder="Aylıq Xerc"
          class="input input-bordered flex-1"
          value={newDersFee() == 0 ? "" : newDersFee()}
          onInput={(e) => setNewDersFee(Number(e.currentTarget.value))}
        />
      ...
      <div class="bg-white shadow-md rounded-lg p-4">
        {loading() ? (
          ...
          <ul class="space-y-2">
            {dersler().map((ders) => (
              <li class="border border-gray-200 rounded-lg p-3 flex justify-between items-center hover:bg-gray-50 text-black transition">
                {editingId() === ders.id ? (
                  <form onSubmit={handleUpdate} class="flex gap-2 w-full">
                    ...
                    <input
                      ...
                      value={editDersFee()}
                      onInput={(e) =>
                        setEditDersFee(Number(e.currentTarget.value))
                      }
                    />
                  ...
                  <>
                    <span>{ders.name}</span>
                    <span>Aylıq Xərc {ders.monthlyfee} AZN</span>
                    <div class="flex gap-2">
                      <button class="btn btn-accent btn-xs">
                        <A href={`/ders/teachers/${ders.id}`}>
                          Müəllimlərə Bax
                        </A>
                      </button>
                      <button class="btn btn-primary btn-xs">
                        <A href={`/ders/students/${ders.id}`}>Tələbələrə Bax</A>
                      </button>
                      <button
